# v0.1 Implementation Plan: Advanced Dada System (ADS)
...
- Verify: define a makefile wrapper for go and other commands implementing golangci-lint v2 and build; 
- Compile: run the makefile, both binaries print help
...
- Verify: `ads new test-session && ads list` shows it
...
- Integration test script: create session → run → produce output → detach → search → verify results
